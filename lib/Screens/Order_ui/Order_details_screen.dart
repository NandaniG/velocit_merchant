...
//   // final Map order;
//   final int basketId;
//   OrderDetailScreen({Key? key, /*required this.order,*/ required this.basketId}) : super(key: key);
//
//
//
//   @override
...
//       data = Provider.of<HomeProvider>(context, listen: false).loadJsonForGetMerchantBasket();
//     });
//     // getColorCodeStatus();
//   }
//
...
//   // var orderStatus = false;
//
//   getColorCodeStatus(HomeProvider value) {
//
//     for(int i = 0; i<value.jsonDataBasket['payload']['merchant_baskets'].length;i++) {
//       List orderList =
//       value.jsonDataBasket['payload']['merchant_baskets'];
//       Map order = orderList[i];
//
//
//       if (order["status_code"] == 1000) {
//         //
...
//           // leadingWidth: width * .06,
//           title: Text(
//             "Basket ID - ${!StringConstant.isLogIn ? '1212' : widget.basketId}",
//             style: const TextStyle(
//                 color: ThemeApp.blackColor,
//                 fontSize: 16,
//                 fontFamily: "Roboto",
...
//     value.jsonDataBasket['payload']['merchant_baskets'];
//     Map order = orderList[i];
//     if(widget.basketId == order['id']){
//
//     getColorCodeStatus(value);
//
//     bottomButtonVisibility(order);
//             return Container(
//                   color: ThemeApp.appBackgroundColor,
//                   width: width,
//                   child: !StringConstant.isLogIn
...
//   }
//
//   Widget authorizedUser(HomeProvider value) {
//     for(int i = 0; i<value.jsonDataBasket['payload']['merchant_baskets'].length;i++) {
//       List orderList =
//       value.jsonDataBasket['payload']['merchant_baskets'];
//       Map order = orderList[i];
...
//
//                             // widget.order["delivery_address_line1"]??""+widget.order["delivery_address_line2"]??""+widget.order["delivery_address_city"]??""+widget.order["delivery_address_pincode"]??""+widget.order["delivery_address_state"]??"",
//                               '${order["delivery_address_line1"] ?? ""},${order["delivery_address_line2"] ?? ""},${order["delivery_address_city"] ?? ""},${order["delivery_address_pincode"] ?? ""},${order["delivery_address_state"] ?? ""}',
//                               softWrap: true,
//                               style: TextStyle(
//                                   color: ThemeApp.primaryNavyBlackColor,
//                                   fontSize: 12,
...
//             itemBuilder: (BuildContext context, int index) {
//               Map orderDetails = order['orders'][index];
//
//               DateFormat format = DateFormat('dd MMM yyyy hh:mm aaa');
//               DateTime date = DateTime.parse(
//                   (orderDetails['delivery_date'] ?? DateTime.now().toString()));
...
//                             height: 15,
//                           ),
//                           orderStatusButtons(order,orderDetails),
//                           SizedBox(
//                             height: 5,
//                           ),
//                           Divider(
...
//   bool isAnyOrderRejected = false;
//
//   Widget orderStatusButtons(Map orderBasket, Map orderDetails) {
//     if (orderDetails["current_status_code"] == 2000) {
//       return Text('');
//     } else {
//       if (orderDetails["is_accepted"] == false) {
...
//                           context,
//                           MaterialPageRoute(
//                               builder: (context) =>  OrderDetailScreen(basketId: widget.basketId,)));
//                     });
//
//                     print("Repository().showSpinner" + showSpinner.toString());
//                     // Provider.of<HomeProvider>(context, listen: false)
...
//   int cntIsCanceled = 0;
//
//   bottomButtonVisibility(Map order) {
//
//
//       for (int i = 0; i < order['orders'].length; i++) {
//       if (order['orders'][i]['is_accepted']) {
//         cntIsAccept = cntIsAccept + 1;
//       }
//
//       if (order['orders'][i]['is_packed']) {
//         cntIsPacked = cntIsPacked + 1;
//       }
//       if (order['orders'][i]['is_shipped']) {
//         cntIsShipped = cntIsShipped + 1;
//       }
//       if (order['orders'][i]['current_status_code'] == 2000) {
//         cntIsCanceled = cntIsCanceled + 1;
//       }
//     }
//     // allBottomButtons();
...
//   }
//
//   Widget allBottomButtons(Map order) {
//
//
//     if (cntIsCanceled == order['orders'].length) {
//       print("step 1");
//       return Container();
//     } else if ((cntIsAccept == order['orders'].length) &&
//         (cntIsPacked == order['orders'].length) &&
//         (cntIsShipped == 0)) {      print("step 2");
//
//     return SafeArea(
//           child: SizedBox(
//         height: 45,
...
//                           .loadJsonForChangeStatus(
//                           710,
//                           order['orders'][i]['merchant_id'],
//                           order['orders'][i]['order_id'],
//                           context);
//                       Timer(const Duration(seconds: 2), () {
//                         setState(() {
//                           showSpinner = false;
...
//       ));
//     } else if ((cntIsAccept == order['orders'].length) &&
//         (cntIsPacked == 0)) { print("step 4");
//       return Container();
//     } else if (cntIsAccept == 0 && cntIsPacked == 0 && cntIsShipped == 0) { print("step 5");
//       return SafeArea(
//           child: SizedBox(
//         height: 45,
//         child: Row(
...
//               children: [
//                 TextFieldUtils().dynamicText(
//                     "${indianRupeesFormat.format(order['total_mrp'])}",
//                     context,
//                     TextStyle(
//                       color: ThemeApp.primaryNavyBlackColor,
//                       fontSize: height * .023,
...
//                       //     "Price (${payload.ordersForPurchase!.length.toString()} items)",
//                       //     context),
//                       Text(indianRupeesFormat.format(order['total_mrp']),
//                           style: TextStyle(
//                               fontFamily: 'Roboto',
//                               color: ThemeApp.lightFontColor,
//                               fontSize: 12,
...
//                               fontWeight: FontWeight.w500)),
//                       Text(
//                           "- ${indianRupeesFormat.format(double.parse(order['total_discount'].toString()))}",
//                           style: TextStyle(
//                               fontFamily: 'Roboto',
//                               color: ThemeApp.lightFontColor,
//                               fontSize: 12,
...
//                                 fontWeight: FontWeight.w700)),
//                         Text(
//                             "${indianRupeesFormat.format(order['total_payable'] ?? 0)} ",
//                             style: TextStyle(
//                                 fontFamily: 'Roboto',
//                                 color: ThemeApp.appColor,
//                                 fontSize: 20,
